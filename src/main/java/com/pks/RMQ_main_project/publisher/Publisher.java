...
public class Publisher {
	...
	@Autowired
	public Publisher(RabbitTemplate rabbitTemplate, AmqpAdmin amqpAdmin,
	...
	public String sendMessage(Object message, String queueName, String exchangeName, String routingKey) {
		log.info("Sending message to queue: {} via exchange: {} with routing key: {}", queueName, exchangeName,
				routingKey);
...
//           x
	...
	/**
	 * Ensures that the required RabbitMQ infrastructure (exchange, queue, binding)
	 * exists
	 */
	private void ensureInfrastructure(String queueName, String exchangeName, String routingKey) {
		try {
			...
			Binding binding = BindingBuilder.bind(queue).to(new TopicExchange(exchangeName)).with(routingKey);
